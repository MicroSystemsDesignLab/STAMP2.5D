...
class PassiveInterposer(System_25D):
	...
	def __init__(self):
		...
		self.power_density = [None] * self.chiplet_count
	...
	def combined_flp(self,filename):
		layer_info = [
            (0, "Substrate", "L0_Substrate.flp", 0.0002),
            (1, "C4Layer", "L1_C4Layer.flp", 0.00007),
            (2, "Interposer", "L2_Interposer.flp", 0.00011),
            (3, "UbumpLayer", "L3_UbumpLayer.flp", 1.00e-05),
            (4, "ChipLayer", "L4_ChipLayer.flp", 0.00015),
            (5, "TIM", "L5_TIM.flp", 2.00e-05),
        ]
		combined_filename = self.path + filename + 'combined_layers.txt'
		with open(combined_filename, 'w') as combined:
			for (layer_no, layer_name, flp_file, thickness) in layer_info:
				# Write the layer header line
				combined.write(f"{layer_no} {layer_name} {thickness}\n")
				# Read the corresponding floorplan file and write its data
				with open(self.path + filename + flp_file, 'r') as flp:
					for line in flp:
						...
						if line_stripped.startswith('#') or line_stripped == '':
							continue
						combined.write(line)
                # Add a blank line after each layer section, if desired
				combined.write("\n")
	...
	def compute_power_density(self):
		chip_thickness = 0.00015  # Fixed thickness from LCF file (in meters)
		power_density_list = []
		for i in range(self.chiplet_count):
			chip_width = self.width[i] / 1000.0  # Convert mm to m
			chip_height = self.height[i] / 1000.0
			volume = chip_width * chip_height * chip_thickness
			pd = self.power[i] / volume  # in W/m^3
			power_density_list.append(pd)
		return power_density_list
	...
	def run_hotspot(self, filename):
		...
		proc.communicate()
		print("HotSpot completed. Exiting for debugging.")		
		exit()
		outlist = stdout.split()
		print("PassiveInterposer.py Line 210 Outlist from run_hotspot: ",outlist) 
