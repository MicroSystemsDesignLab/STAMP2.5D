...
def solve_Cplex(system):
	...
	# calculate d #### Equation2 in paper
	...
	# Eq. 11. initialize f[i][h][j][k][n] and set lower bound 0 ###Eqn3 in paper
	for i in range(Nchiplet):
		for h in range(Nclump):
			for j in range(Nchiplet):
				for k in range(Nclump):
			...
			for j in range(Nchiplet):
				if j != s[n]: # This is to make sure there is no duplicate indices which raises an exception.
					for k in range(Nclump):
						...
						row_coeff.append(-1)
		problem.linear_constraints.add(lin_expr = [[row_index, row_coeff]], senses = ["E"], rhs = [R[s[n]][t[n]]]) ####Eqn3 (1)
			...
			for j in range(Nchiplet):
				if j != t[n]: # This is to make sure there is no duplicate indices which raises an exception.
					for k in range(Nclump):
						...
						row_index.append(fji_index)
						row_coeff.append(-1)
		problem.linear_constraints.add(lin_expr = [[row_index, row_coeff]], senses = ["E"], rhs = [-R[s[n]][t[n]]]) #####Eqn 3 (2)
					...
					for j in range(Nchiplet):
						if j != i: # This is to make sure there is no duplicate indices which raises an exception.
							for k in range(Nclump):
								...
								row_index.append(fji_index)
								row_coeff.append(-1)
				problem.linear_constraints.add(lin_expr = [[row_index, row_coeff]], senses = ["E"], rhs = [0]) ##### Eqn 3 (3)
