...
base_dir   = r"<path_to_your_workspace>\AI_Temperature\Ascend910_july10"
...
def therm_mech(layer_file, power):
    ...
    process_ansys_iteration(
        xls_file_path=os.path.join(xls_dir, f"{arch}_34402_{index}_temp.xls"),
        chiplets=chiplets,
...
def geometry_from_layers(file_contents):
    ...
    path_ = r"<path_to_your_workspace>\AI_Temperature\Ascend910_Iter1"
...
geometryPath = r"<path_to_your_workspace>\\AI_Temperature\\Ascend910_Iter1\\geometry_export.step"
...
""")
    ...
    index+=1
    txt_path_temp = os.path.join(xls_dir, f"{arch}_34402_{index}_temp.xls")
    print(txt_path_temp)
