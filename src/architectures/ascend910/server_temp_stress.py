...
sys.path.append(r"<path_to_your_workspace>\RL_Planner")
base_dir   = r"<path_to_your_workspace>\RL_Planner\Ascend910_july10"
xls_dir = os.path.join(base_dir, "ansys_temp_stress")
input_dir= os.path.join(base_dir, "input_temp_stress")
target_dir = os.path.join(base_dir, "target_ts_temp")
target_stress_dir = os.path.join(base_dir, "target_ts_stress")
os.makedirs(input_dir,  exist_ok=True)
os.makedirs(target_dir, exist_ok=True)
os.makedirs(xls_dir,  exist_ok=True)
os.makedirs(target_stress_dir, exist_ok=True)
#from dataset_gen import *
#from ansys_io_arch1 import *
index=0
...
def home():
    ...
    return "STAMP-2.5D local server is running."
...
def therm_mech(layer_file, power):
    ...
    process_ansys_iteration(
        xls_file_path=os.path.join(xls_dir, f"{arch}_34403_{index}_temp.xls"),
        chiplets=chiplets,
        architecture_name=arch,
        sample_id=id,
        interposer_size_mm=45.0,
        input_dir=input_dir,
...
def geometry_from_layers(file_contents):
    ...
    path_ = r"<path_to_your_workspace>\AI_Temperature\Ascend910_july9"
...
geometryPath = r"<path_to_your_workspace>\\AI_Temperature\\Ascend910_july9\\geometry_export.step"
...
convection_.FilmCoefficient.Output.SetDiscreteValue(0, Quantity(580, "W m^-1 m^-1 C^-1"))
selection = ExtAPI.SelectionManager.CreateSelectionInfo(SelectionTypeEnum.GeometryEntities)
selection.Ids = [1929]
...
""")
    ...
    Structural_analysis_script = dedent("""\
...
tdeform = ss1.Solution.AddTotalDeformation()
...
ansys_analysis_settings = Model.Analyses[1].AnalysisSettings
ansys_analysis_settings.SolverType = SolverType.ProgramControlled
step_index_list = [1]
with Transaction():
    for step_index in step_index_list:
        ansys_analysis_settings.SetAutomaticTimeStepping(step_index, AutomaticTimeStepping.Off)
ss1.Solve()
""")
    mechanical.run_python_script(Structural_analysis_script)
    mechanical.log_message("INFO", "Stress_thermal_done")
    arch="ascend910" 
    global index,xls_dir
    index+=1
    txt_path_temp = os.path.join(xls_dir, f"{arch}_34403_{index}_temp.xls")
    print(txt_path_temp)
    text_path_stress = os.path.join(xls_dir, f"{arch}_34403_{index}_stress.xls")
    global csv_file
    export_script = dedent(fr"""
import os, re
ExtAPI.Application.ActiveUnitSystem = MechanicalUnitSystem.StandardNMM
Model.Analyses[0].Solution.Children[1].ExportToTextFile(r"{txt_path_temp}")
Model.Analyses[1].Solution.Children[2].ExportToTextFile(r"{text_path_stress}")
""")
    mechanical.run_python_script(export_script)
    mechanical.exit()


def ansys_therm_mech_2():
    from ansys.mechanical.core import launch_mechanical, App, find_mechanical, global_variables
    from textwrap import dedent
    mechanical = launch_mechanical(
        exec_file=None,
    ...
    script_GeometryImport = dedent("""\
geomImport = Model.GeometryImportGroup.AddGeometryImport()
geometryPath = r"<path_to_your_workspace>\\AI_Temperature\\geometry_export.step"
geomImport.Import(geometryPath)
""")
    mechanical.run_python_script(script_GeometryImport)
    mechanical.log_message("INFO", "Geometry Import Success")
...
""")
    thermal_analysis += dedent("""\
S_internal_heat_0 = S_therm.AddInternalHeatGeneration()
S_internal_heat_0.Magnitude.Output.SetDiscreteValue(0, Quantity(4.86e9, "W m^-1 m^-1 m^-1"))
selection = ExtAPI.SelectionManager.CreateSelectionInfo(SelectionTypeEnum.GeometryEntities)
selection.Ids = [802]
S_internal_heat_0.Location = selection
""")
    thermal_analysis += dedent("""\
S_internal_heat_1 = S_therm.AddInternalHeatGeneration()
S_internal_heat_1.Magnitude.Output.SetDiscreteValue(0, Quantity(5.94e9, "W m^-1 m^-1 m^-1"))
selection = ExtAPI.SelectionManager.CreateSelectionInfo(SelectionTypeEnum.GeometryEntities)
selection.Ids = [829]
S_internal_heat_1.Location = selection
""")
    thermal_analysis += dedent("""\
S_internal_heat_2 = S_therm.AddInternalHeatGeneration()
S_internal_heat_2.Magnitude.Output.SetDiscreteValue(0, Quantity(5.94e9, "W m^-1 m^-1 m^-1"))
selection = ExtAPI.SelectionManager.CreateSelectionInfo(SelectionTypeEnum.GeometryEntities)
selection.Ids = [856]
S_internal_heat_2.Location = selection
""")
...
""")
    ...
    thermal_analysis = dedent("""\
convection_ = S_therm.AddConvection()
convection_.FilmCoefficient.Output.SetDiscreteValue(0, Quantity(950, "W m^-1 m^-1 C^-1"))
selection = ExtAPI.SelectionManager.CreateSelectionInfo(SelectionTypeEnum.GeometryEntities)
selection.Ids = [2025]
convection_.Location = selection
Model.Analyses[0].Solution.AddTemperature()
...
""")
    mechanical.run_python_script(Structural_analysis_script)
    mechanical.log_message("INFO", "strucutral done")
    arch="multi_gpu" 
    global index,xls_dir
    index+=1
    ...
    #csv_path   = "<path_to_your_repo>\\data\\exp_arch1_result\\results_test1.csv"
...
def ansys_therm_mech_3():
    ...
    script_GeometryImport = dedent("""\
geomImport = Model.GeometryImportGroup.AddGeometryImport()
geometryPath = r"<path_to_your_workspace>\\AI_Temperature\\Micro150_updated\\geometry_export.step"
geomImport.Import(geometryPath)
""")
    mechanical.run_python_script(script_GeometryImport)
    mechanical.log_message("INFO", "Geometry Import Success")
...
""")
    ...
    thermal_analysis = dedent("""\
convection_ = S_therm.AddConvection()
convection_.FilmCoefficient.Output.SetDiscreteValue(0, Quantity(720, "W m^-1 m^-1 C^-1"))
selection = ExtAPI.SelectionManager.CreateSelectionInfo(SelectionTypeEnum.GeometryEntities)
selection.Ids = [2183]
convection_.Location = selection
Model.Analyses[0].Solution.AddTemperature()
...
""")
    mechanical.run_python_script(Structural_analysis_script)
    mechanical.log_message("INFO", "stress_done")
    # Export results to text file
    arch="micro150" 
    global index,xls_dir
    index+=1
    txt_path_temp = os.path.join(xls_dir, f"{arch}_34401_{index}_temp.xls")
    text_path_stress = os.path.join(xls_dir, f"{arch}_34401_{index}_stress.xls")
    global csv_file
    export_script = dedent(fr"""
import os, re
ExtAPI.Application.ActiveUnitSystem = MechanicalUnitSystem.StandardNMM
